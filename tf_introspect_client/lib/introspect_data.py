...
class BgpPeerInfoData(IntrospectData):
    def __init__(self, obj):
        ...
        self.PeerStateInfo = PeerStateInfo(self.obj)


class XmppPeerInfoData(IntrospectData):
    def __init__(self, obj):
        super(XmppPeerInfoData, self).__init__(obj)
        self.PeerEventInfo = PeerEventInfo(self.obj)
        self.PeerStateInfo = PeerStateInfo(self.obj)


class PeerEventInfo(IntrospectData):
    def __init__(self, obj):
        try:
            super(PeerEventInfo, self).__init__(
                obj['event_info'][0][0]['PeerEventInfo'])
        except KeyError:
            self.obj = None

    @property
    def last_event(self):
        return self.obj.get('last_event').get('#text')

    @property
    def last_event_at(self):
        return self.obj.get('last_event_at').get('#text')


class PeerStateInfo(IntrospectData):
    def __init__(self, obj):
        try:
            super(PeerStateInfo, self).__init__(
                obj['state_info'][0][0]['PeerStateInfo'])
        except KeyError:
            self.obj = None
